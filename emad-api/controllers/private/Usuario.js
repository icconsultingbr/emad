module.exports = function (app) {
    ...
    app.post('/usuario', function (req, res) {
        ...
        if (usuario.idTipoUsuario == util.SUPER_ADMIN || (usuario.idTipoUsuario == util.ADMIN && cadastro.idTipoUsuario != util.SUPER_ADMIN)) {
            ...
            req.assert("sexo").notEmpty().withMessage("Sexo é um campo obrigatório").isLength({ min: 1, max: 1 }).withMessage("Sexo deve ter apenas um caractere;");
            ...
            req.assert("cpf").isLength({ min: 11, max: 11 }).withMessage("CPF deve conter 11 caracteres;").custom(util.cpfValido).withMessage("CPF inválido;");
            ...
            buscaPorEmail(cadastro).then(function (responseEmail) {

                if (responseEmail.length > 0) {
                    errors = util.customError(errors, "email", "Já existe um usuário cadastrado com este e-mail!", cadastro.email);
                ...
                if (responseCPF.length > 0) {
                    ...
                    errors = util.customError(errors, "cpf", "Já existe um usuário cadastrado com este cpf!", cadastro.email);
                ...
                for (var i = 0; i < estabelecimentos.length; i++) {
                    arrEstabelecimentos.push("(" + cadastro.id + ", " + estabelecimentos[i].id + ")");
                }
    ...
    app.put('/usuario', function (req, res) {
        ...
        if (
            ...
            (usuario.idTipoUsuario == util.ADMIN && cadastro.idTipoUsuario != util.SUPER_ADMIN)) {
            ...
            buscaPorEmail(cadastro).then(function (responseEmail) {
                if (responseEmail.length > 0) {
                    errors = util.customError(errors, "email", "Este e-mail não está disponível para cadastro!", cadastro.email);
                    //console.log('Email já existe'+errors);
                    res.status(404).json(errors);
                    return;
                }
                ...
                if (responseCPF.length > 0) {
                    errors = util.customError(errors, "cpf", "Já existe um usuário cadastrado com este cpf!", cadastro.cpf);
                    //console.log('CPF já existe'+errors);
                    res.status(404).json(errors);
                    return;
                }
                ...
                for (var i = 0; i < estabelecimentos.length; i++) {
                    arrEstabelecimentos.push("(" + id + ", " + estabelecimentos[i].id + ")");
                }
    ...
    app.put('/usuario/redefinir-senha', function (req, res) {
        ...
        req.assert("senhaAtual").notEmpty().withMessage("A senha atual é um campo obrigatório;");
        ...
        buscaPorId(usuario.id, res).then(function (response) {
            ...
            if (util.checkPassword(body.senhaAtual, usuario.senha)) {
                ...
                errors = util.customError(errors, "data", "As senhas não conferem", "usuarios");
        ...
        if (usuario.id != id) {
            ...
            buscaPorId(id, res).then(function (responseUsuario) {
                if (responseUsuario.id) {
                    ...
                    if (usuario.idTipoUsuario == util.SUPER_ADMIN || (usuario.idTipoUsuario == util.ADMIN)) {
                    ...
                    errors = util.customError(errors, "header", "Usuário não encontrado", "acesso");
                    res.status(401).send(errors);
                }
            });
        } else {
            errors = util.customError(errors, "header", "Você não pode apagar seu próprio usuário", "acesso");
            res.status(401).send(errors);
        }
    });
    ...
    function buscaUsuarioPorEmpresa(usuario, id, res) {
        ...
        usuarioDAO.buscaUsuarioPorEmpresaId(usuario, id, function (exception, result) {
            if (exception) {
                d.reject(exception);
                console.log(exception);
                errors = util.customError(errors, "data", "Erro ao acessar os dados", "usuarios");
    ...
    function checaExtrato(objeto, res) {
        ...
        objDAO.checaSaldo(objeto, function (exception, result) {
            if (exception) {
                d.reject(exception);
                console.log(exception);
                errors = util.customError(errors, "data", "Erro ao acessar os dados", "extrato");
                res.status(500).send(errors);
                return;
            } else {
    ...
    function deletaPorId(id, res) {
        ...
        objDAO.deletaPorId(id, function (exception, result) {
            if (exception) {
                d.reject(exception);
                errors = util.customError(errors, "data", "Erro ao remover os dados", "usuario");
                res.status(500).send(errors);
                return;
            } else {
