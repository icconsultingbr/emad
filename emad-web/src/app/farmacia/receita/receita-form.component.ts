...
export class ReceitaFormComponent implements OnInit {
  ...
  carregaEstoque(item: any) {
    ...
    item.expandir = !item.expandir;
    const params = '?idReceita=' + item.idReceita + '&idItemReceita=' + item.id;
  ...
  estoqueContemDivergencias() {
    ...
    this.listaMaterialLote.forEach(item => {
      if (item.qtdDispensar > 0 && item.qtdDispensar != 'undefined') {
        ...
        const medicamentoExistenteComLote = listaMaterialLoteExistente.filter(itemAdicionado => itemAdicionado.idMaterial == this.itemReceita.idMaterial
    ...
    const medicamentoExistenteSemLote = listaMaterialLoteExistente.filter(itemAdicionado => itemAdicionado.idMaterial == this.itemReceita.idMaterial && !itemAdicionado.lote && !itemAdicionado.idFabricanteMaterial);
